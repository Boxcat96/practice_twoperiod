"""
Purpose:
Solve two period model using optimization tool.
...
# use optimization tool in scipy

pol_a2 = np.zeros(na)

for i in range(na):
    arg = (y1, y2, grid_a1[i], beta, gamma, rent, )
    pol_a2[i] = fmin(mef.obj_two_period, [0.0], args=arg)
...
plt.grid(True)
plt.savefig('Fig2_maximization.pdf')
plt.show()

# plot closed- form solution

# coefficients of closed-form solution
